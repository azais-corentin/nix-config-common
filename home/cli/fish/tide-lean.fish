# Generated snapshot of the Tide Lean preset.
# Source command:
#   tide configure --auto --style=Lean --prompt_colors='True color' \
#     --show_time='24-hour format' --lean_prompt_height='One line' \
#     --prompt_spacing=Compact --icons='Few icons' --transient=Yes
#
# Loaded from conf.d on every interactive fish shell. Uses set -g so the
# values shadow any stale universals from prior "tide configure" runs
# without writing back to fish_variables.
#
# To regenerate: re-run the tide configure command above, then in fish run
#   set -U -n | string match -er ^tide_ | sort
# and re-emit one set -g line per variable, single-quoting each value.
